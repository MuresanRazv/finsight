...
export function Sidebar({ session }: { session?: SessionUser }) {
    ...
    return (
        <>
            ...
            <aside
                className={cn(
                    'border-border bg-sidebar fixed inset-y-0 left-0 z-50 flex w-64 shrink-0 flex-col justify-between border-r px-4 py-6 transition-transform duration-300 ease-in-out md:static md:translate-x-0 md:flex',
            ...
            >
                ...
                <div className='px-2'>
                    {pathname !== '/search' && (
                        <Link href='/search' onClick={() => setIsOpen(false)} className='block w-full'>
